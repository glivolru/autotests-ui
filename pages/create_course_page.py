...
class CreateCoursePage(BasePage):
    ...
    def check_visible_image_upload_view(self, is_image_uploaded: bool = False):
        ...
        expect(self.preview_img_upload_description).to_have_text(
            'Recommended file size 540X300'
        )

        expect(self.preview_img_upload_button).to_be_visible()

        if is_image_uploaded:
            expect(self.preview_img_upload_button).to_be_visible()

    def click_remove_image_button(self):
        self.preview_img_remove_button.click()

    def check_visible_preview_image(self):
        expect(self.preview_img).to_be_visible()

    def upload_preview_image(self, file: str):
        self.preview_img_upload_button.set_input_files(file)
